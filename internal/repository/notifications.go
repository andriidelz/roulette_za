...
func (r *PostgresRepository) GetUsersForNotificationTask(task *models.NotificationTask) ([]models.User, error) {
	...
	if task.TargetType != "unregistered" {
		query = query.Where("status = ?", config.UserStatusActive)
	}
	...
	case "unregistered":
		...
		query = query.Where("status != ?", config.UserStatusDisabled).Where("registered = ?", false)

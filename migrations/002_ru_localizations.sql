...
Здесь вы можете изменить настройки профиля:'),
    ...
    ('withdrawusdtchange', 'ru', 'Укажите в ответ новый адрес для вывода USDT TRC20 одним сообщением без пробелов. Проверьте внимательно адрес – отправку USDT на неверный адрес невозможно отменить.'),
    ('withdrawusdtchangeok', 'ru', 'Ваш адрес USDT TRC20 для вывода баланcа успешно изменен.'),
    ('withdrawusdtchangeerror', 'ru', 'Вы ввели некорректный адрес USDT TRC20. Проверьте адрес еще раз и отправьте его в следующем сообщении.'),
    ...
    ('statistics_next', 'ru', 'Выберите другой временной период для получения статистики или вернитесь в главное меню'),
...
Вам не хватает %d баллов, чтобы принять участие в распределении еженедельной награды.'),
    ...
    ('username_points', 'ru', '<b>%s</b> - %d баллов'),
    ('username_points_efficiency', 'ru', '<b>%s</b> - %d баллов (%.1f%%)'),
    ...
    ('withdrawusdtcheck', 'ru', 'В вашем аккаунте указан такой адрес для вывода USDT <pre>%s</pre>. Проверьте и подтвердите адрес или измените его. Отправку USDT на неверный адрес невозможно отменить.'),
    ('usdtok', 'ru', 'Подтвердить адрес'),
    ('usdtchange', 'ru', 'Изменить адрес'),
    ('no_wallet_address', 'ru', 'Для вывода необходимо указать адрес кошелька в настройках профиля.'),
    ('withdrawcancel', 'ru', 'Отменить ввод'),
    ('withdrawusdtsumcheck', 'ru', 'Адрес успешно подтвержден. Для вывода доступно %.2f USDT. Комиссия за транзакцию - %.2f USDT - будет списана с суммы вывода. Выведите весь доступный баланс или введите в ответ сумму для вывода, используя только цифры, не превышающую сумму на балансе.'),
    ('withdrawusdtall', 'ru', 'Все'),
    ('withdrawusdtamount', 'ru', 'Ввести сумму'),
    ('withdrawusdtsumam', 'ru', 'Напишите в ответ сумму для вывода, используя только цифры, не превышающую сумму на балансе'),
    ('withdrawusdtsumerror', 'ru', 'Сообщение некорректно, пожалуйста, используйте для ввода только цифры'),
    ('withdrawusdtsumstop', 'ru', 'Если вы хотите остановить вывод, вы можете отменить ввод и остаться в текущем меню или вернуться в главное меню.'),
    ('withdrawusdtsumbig', 'ru', 'К сожалению, сумма на вашем балансе, доступная для вывода - меньше. Пожалуйста, напишите в ответ сумму для вывода, используя только цифры, не превышающую сумму на балансе.'),
    ...
    ('withdrawsumok', 'ru', 'Запрос на вывод %.2f USDT успешно создан. Средства будут отправлены на кошелек %s в течение 24 часов.');
...
INSERT INTO localizations (key, language, value) VALUES
    ...
    ('balance_updated_message', 'ru', 'Вы получили вознаграждение в размере {amount} на внутренний баланс! Ваш текущий баланс: {balance}.'),
    ('top_rating_title', 'ru', 'Еженедельный топ рейтинг'),
    ('top_rating_message', 'ru', 'Поздравляем! Вы вошли в еженедельный топ рейтинг на позиции {position}! Вы набрали {points} очков. Общий призовой фонд составляет {prize_fund}.'),
    ...
    ('name_changeno_msg_start', 'ru', '✅ <b>Твое игровое имя сохранено!</b> Чтобы бесплатно принять участие в розыгрыше еженедельного призового фонда, подпишись на наш резервный канал. Он используется только в случае проблем с доступом к основному боту, чтобы мы всегда оставались на связи.'),
    ...
    ('name_changesave_msg_start', 'ru', '✅ <b>Новое игровое имя для рейтинга сохранено!</b> Чтобы бесплатно принять участие в борьбе за еженедельный призовой фонд, подпишись на наш резервный канал, чтобы оставаться на связи. Он используется только в случае проблем с доступом к основному боту, чтобы мы всегда оставались на связи.'),
...
INSERT INTO localizations (key, language, value) VALUES
    ...
    ('send_error', 'ru', 'К сожалению Вы слишком часто взаимодействуете с ботом, пожалуйста подождите.'),
    ('send_queue_error', 'ru', 'К сожалению из за проблем с производительностью скопилось %d сообщений которые ожидают отправки к Вам, пожалуйста подождите окончания их обработки.'),
    ('captcha_text', 'ru', 'Ваша активность очень высока. Пожалуйста подтвердите что вы не робот.'),
    ('captcha_correct', 'ru', 'Поздравляем с успешным прохождением капчи! На будущее постарайтесь взаимодействовать с ботом аккуратнее'),
    ('startmessage1_new', 'ru', 'Поздравляем тебя в 🐙 <b>Sprut Black & Red</b>
Это бесплатная игра в Telegram-боте, которая еженедельно позволяет выигрывать <b><i>💵реальные финансовые призы</i></b> за виртуальные угадывания!

1️⃣ Регистрируйся в сапоге
2️⃣ Угадай, что выпадет на рулетке - 🔴красное, ⚫️черное или 🟢Zero
3️⃣ Получай баллы в рейтинг за верное угадывание, чтобы получить большее вознаграждение.

📢<b> Sprut Black & Red имеет уникальные преимущества:</b>
▪️<i>Еженедельный призовой фонд, выплаты по понедельникам;</i>
▪️<i>Отсутствие верификации;</i>
▪️<i>Отсутствие депозитов или какой-либо оплаты за участие</i>
▪️ <i>Возможность моментального вывода вознаграждения - никаких airdropiв, токенов и долгих ожиданий;</i>
▪️ <i>Прозрачная система розыгрыша, с возможностью независимой проверки каждого угадывания.</i>

❗️Это не шутка и не кликбейт. Просто крути рулетку в 🐙 <b>Sprut Black & Red</b> — без вложений, без рисков, с шансом разделить 💰призовой фонд в {prize_fund}$ еженедельно!

👇 <b>Чтобы начать — нажми кнопку <u>«Регистрация»</u> для подтверждения возраста</b>'),
    ('btn_reg', 'ru', '🐙 Завершить регистрацию'),
    ('bet_inactive', 'ru', 'Принятие ставок остановлено из-за вашего бездействия. Чтобы начать играть - нажмите кнопку Играть в меню.');